...
import sumosim.robot.sensors.Sensor;

public class SumoRobot extends Scene {
	...
	protected void init(World world) {
		...
		border = new StaticBody(SumoRobot.DOHYO_BORDER, new Circle(240));
		border.setPosition(250, 270);
		arena = new StaticBody(SumoRobot.DOHYO_ARENA, new Circle(220));
		arena.setPosition(250, 270);
				
		r1 = new Robot(world, 250, 200, 0, 10);
		r2 = new Robot(world, 250, 360, (float)Math.PI, 10);
	...
	protected void update() {
		...
		if (Collide.collide(new Contact[] { new Contact(), new Contact() }, r1.getHull(), arena, 0f) == 0) {
			needsReset = true;
		}
		if (Collide.collide(new Contact[] { new Contact(), new Contact() }, r2.getHull(), arena, 0f) == 0) {
			needsReset = true;
		}
	}
